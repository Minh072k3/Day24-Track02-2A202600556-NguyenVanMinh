...
def validate_anonymized_data(filepath: str) -> dict:
    ...
    if any(str(cccd) in original_cccds for cccd in df["cccd"]):
        results["success"] = False
        results["failed_checks"].append("CCCD gốc bị rò rỉ")

    # Check 2: Không có null values trong các cột quan trọng
    important_cols = ["patient_id", "ho_ten", "cccd", "benh", "ket_qua_xet_nghiem"]
    if df[important_cols].isnull().any().any():
        results["success"] = False
        results["failed_checks"].append("Cột quan trọng chứa giá trị Null")

    # Check 3: Số rows phải bằng original
    if len(df) != len(original_df):
        results["success"] = False
        results["failed_checks"].append("Số lượng dòng bị thay đổi")
